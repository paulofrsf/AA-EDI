...
void analyzeCompressionRate(float compressionRate) {
	...
	if (compressionRate >= 75) {
		printf("File size reduced dramaticaly! *___*");
	}
	else if (compressionRate >= 50) {
		printf("Great, satisfatory file compression! :O");
	}
	else if (compressionRate >= 25) {
		printf("Maybe there are better cases, but it's still a surplus. :D");
	}
	else {
		printf("It seems you built a huffman linked list, not actually a tree. :S");
	}
}
...
struct huffmanData * compress(char * toCompress) {
	...
	printf("Getting character frequency data...\n\n");
	...
	printf("\nBuilding huffman tree and encoding...\n\n");
